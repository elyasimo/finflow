...
export default function MobileHeader({ user, title, showLogo = true }: MobileHeaderProps) {
  ...
  return (
    <header 
      className="lg:hidden flex-shrink-0 bg-white/95 dark:bg-[#0f1623]/95 backdrop-blur-xl border-b border-gray-200/50 dark:border-[#232e40]/50 pt-[env(safe-area-inset-top)] mobile-header app-header"
      style={{ 
        width: '100%',
        maxWidth: '100vw', 
        overflowX: 'hidden',
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2 min-w-0 flex-shrink" style={{ maxWidth: '35%' }}>
          {canGoBack ? (
            <button 
              onClick={() => router.back()}
